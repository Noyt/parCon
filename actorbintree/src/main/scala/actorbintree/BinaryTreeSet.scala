...
class BinaryTreeNode(val elem: Int, initiallyRemoved: Boolean) extends Actor {
  ...
  def nextHop(e: Int): Position = if (e > elem) Right else Left

  // optional
  /** Handles `Operation` messages and `CopyTo` requests. */
  val normal: Receive = {
    case Contains(requester, id, to_find) =>
      if (to_find != elem || (to_find == elem && removed)) subtrees get nextHop(to_find) match {
    ...
    case Insert(requester, id, to_insert) =>
      if (to_insert != elem || (to_insert == elem && removed)) subtrees get nextHop(to_insert) match {
        case Some(subtree) => subtree ! Insert(requester, id, to_insert)
        case None =>
          subtrees += (nextHop(to_insert) -> context.actorOf(BinaryTreeNode.props(to_insert, initiallyRemoved = false)))
          requester ! OperationFinished(id)
      } else requester ! OperationFinished(id)

    case Remove(requester, id, to_delete) =>
      if (to_delete != elem || (to_delete == elem && removed)) subtrees get nextHop(to_delete) match {
