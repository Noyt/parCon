...
object WikipediaRanking {
  ...
  def occurrencesOfLang(lang: String, rdd: RDD[WikipediaArticle]): Int = {
    rdd.aggregate(0)((acc, art) => if(art.mentionsLanguage(lang)) acc+1 else acc, _+_)
  }
  ...
  def rankLangs(langs: List[String], rdd: RDD[WikipediaArticle]): List[(String, Int)] = {
    var res: List[(String, Int)] = Nil
    for(lang <- langs){
      val count = occurrencesOfLang(lang, rdd)
      if(count > 0) res = (lang, count) :: res
    }
    res.sortBy(-_._2)
  }
  ...
  def makeIndex(langs: List[String], rdd: RDD[WikipediaArticle]): RDD[(String, Iterable[WikipediaArticle])] = {
    rdd.flatMap( (art) => {
        var list: List[(String, WikipediaArticle)] = Nil
        for(lang <- langs){
          if(art.mentionsLanguage(lang)) list = (lang, art) :: list
        }
        list
      }
    ).groupByKey
  ...
  def rankLangsUsingIndex(index: RDD[(String, Iterable[WikipediaArticle])]): List[(String, Int)] = {
    index.mapValues((list: Iterable[WikipediaArticle]) => list.size).collect.toList.sortBy(-_._2)
  }
  ...
  def rankLangsReduceByKey(langs: List[String], rdd: RDD[WikipediaArticle]): List[(String, Int)] = {
    rdd.flatMap( (art) => {
        ...
        for(lang <- langs){
          if(art.mentionsLanguage(lang)) list = (lang, 1) :: list
        }
        list
      }
