...
class KMeans {
  ...
  def classify(points: GenSeq[Point], means: GenSeq[Point]): GenMap[Point, GenSeq[Point]] = {
    val closest = points.groupBy(x => findClosest(x, means))
    means.map((x: Point) => (x, closest.getOrElse(x, GenSeq()))).toMap
  }
  ...
  def converged(eta: Double)(oldMeans: GenSeq[Point], newMeans: GenSeq[Point]): Boolean = {
    val sqDist = (oldMeans zip newMeans) map ((x, y) => x squareDistance y)
    (sqDist map (x => x <= eta)).foldLeft(true){
      case (a, b) => a && b
    }
  }
