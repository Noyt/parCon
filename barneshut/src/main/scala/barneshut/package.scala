...
package object barneshut {
  ...
  extends Quad {
    /* val (mass, massX, massY) = (
      bodies.foldLeft(0f){(z, b: Body) => z + b.mass},
      bodies.foldLeft(0f){(z, b: Body) => z + b.mass*b.x}/mass,
      bodies.foldLeft(0f){(z, b: Body) => z + b.mass*b.y}/mass
      ) */

    val mass = bodies.foldLeft(0f){(z, b: Body) => z + b.mass}
    val massX = bodies.foldLeft(0f){(z, b: Body) => z + b.mass*b.x}/mass
    val massY = bodies.foldLeft(0f){(z, b: Body) => z + b.mass*b.y}/mass
    
    val total: Int = bodies.length
    def insert(b: Body): Quad = {
      if(size > minimumSize){
        val tmp = Fork(
          Empty(centerX - 0.25f*size, centerY - 0.25f*size, size/2f),
          Empty(centerX + 0.25f*size, centerY - 0.25f*size, size/2f),
          Empty(centerX - 0.25f*size, centerY + 0.25f*size, size/2f),
          Empty(centerX + 0.25f*size, centerY + 0.25f*size, size/2f)
          )
        (bodies:+b).foldLeft(tmp){(z, bo)=> tmp.insert(bo)}
        //b +: bodies foreach tmp.insert
        //tmp
      } else {
  ...
  class SectorMatrix(val boundaries: Boundaries, val sectorPrecision: Int) {
    ...
    def +=(b: Body): SectorMatrix = {
      /* val x: Int = (0 to sectorPrecision).indexWhere(i => boundaries.minX + i * sectorSize > b.x) - 1
      val y: Int = (0 to sectorPrecision).indexWhere(i => boundaries.minY + i * sectorSize > b.y) - 1
      matrix(y * sectorPrecision + x) = matrix(y * sectorPrecision + x) += b
      this */
